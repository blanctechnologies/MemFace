...
class NeuralRenderingDataset(Dataset):
    def __init__(
        self,
        data_dir='/home/avocoral/Downloads/Obamaset/Obama_vid/Obama',
        metadata_dir='/home/avocoral/Downloads/Obamaset/Obama_meta',
    ):
    ...
    def __getitem__(self, idx):
        ...
        # logger.info(f'print dataset len: {self.__len__()}')
        # logger.info(f'frames_dir: {frames_dir}')
        orig_images = []
        landmarks3d_final = []
        masked_ref_images = []
        logger.info(f'len(frames_dir): {len(frames_dir)}')
        for i, frame_dir in enumerate(frames_dir):
            ...
            # logger.info(f'original_image.shape: {original_image.shape}')
            ref_image = cv2.imread(ref_path)[:450, :450, :]
            # logger.info(f'ref_image.shape: {ref_image.shape}')
            mask_image = cv2.resize(
                cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE),
                (self.resolution, self.resolution),
                interpolation=cv2.INTER_LINEAR,
            )
            # logger.info(f'mask_image.shape: {mask_image.shape}')
            ...
            mouth_landmarks = torch.Tensor(
                [[[-int(objects[0][i][0]), -int(objects[0][i][1])] for i in range(len(objects[0])) if 48 <= i < 69]]
            )
        ...
        logger.info(
            f'masked_ref_images.shape, orig_images.shape, landmarks3d.shape = {masked_ref_images.shape}, {orig_images.shape}, {landmarks3d.shape}'
        )
...
class Audio2ExpDataset(Dataset):
    def __init__(self, audio_embed_dir: str, coeff_dir: str, transform=None, stage=None):
    ...
    def __getitem__(self, idx):
        logger.info(f'ℹ️  idx: {idx}')
        sample_name = os.listdir(self.coeff_dir)[idx]
        logger.info(f'Dataloader stage: /{self.stage}/')
        if self.stage == 'fit':
        ...
        # logger.info(f'foldername = {sample_name[:-3]}')
        # logger.info(f'coeff_folderpath: {coeff_folderpath}')
        # audioembeds every 50ms, so 1 second of audio is 50 elements, 30 seconds is 1500
        # logger.info(f'audio_embed: {audio_embed}')
        ...
        for i, frame_name in enumerate(os.listdir(coeff_folderpath)):
            if i > 900 and self.__len__() == 1:
                # adaptation, hence take only 30 sec
                # need to differ adaptation and inference?
                break
            ...
            # logger.info(f'landmarks_frame.shape: {Om_frame.shape}')
            # [:, 48:, :]
            Om.append(
                torch.from_numpy(np.load(os.path.join(coeff_folderpath, frame_name, 'landmarks3d.npy')))[:, 48:, :]
            )

        # logger.info(f'Om[0].shape: {Om[0].shape}')
        ...
        # logger.info(f'video_frames_num: {video_frames_num}')
        audio_embed = F.interpolate(audio_embed.T.unsqueeze(0), size=[video_frames_num], mode='nearest').squeeze(0).T

        # logger.info(f'new audio_embed shape orig: {audio_embed.shape}')
        logger.info(f"exp_coeff shape orig: {exp.shape}")
        # logger.info(f"pose_coeff shape orig: {pose.shape}")
        # logger.info(f"shape_coeff shape orig: {shape.shape}")
        # logger.info(f"landmarks3d shape orig: {landmarks3d.shape}")
...
class Audio2ExpDataModule(pl.LightningDataModule):
    def __init__(
        self,
        audio_dir: str = '/mnt/sda/AVSpeech/dataset_v2_audio',
        coeff_dir: str = '/mnt/sda/AVSpeech/dataset_v2_video',
        batch_size: int = 16,
    ):
        super().__init__()
    ...
    def setup(self, stage='fit'):
        if stage == 'fit':
            AVSpeech = Audio2ExpDataset(self.audio_dir, self.coeff_dir, stage='fit')
            # proportions = [0.85, 0.15]
            # lengths = [int(p * len(AVSpeech)) for p in proportions]
            # lengths[-1] = len(AVSpeech) - sum(lengths[:-1])
            train_ratio = 0.85
            av_speech_size = len(AVSpeech)
            train_size = int(av_speech_size * train_ratio)
            self.train, self.val = random_split(AVSpeech, [train_size, (av_speech_size - train_size)])
            # self.train = AVSpeech
        ...
        #           self._has_setup_fit = True
            ...
            AVSpeech = Audio2ExpDataset(self.audio_dir, self.coeff_dir, stage='adaptation')
            self.train = AVSpeech
            # self.val = AVSpeech[1]
    ...
    def train_dataloader(self):
        return DataLoader(
            self.train, batch_size=self.batch_size, num_workers=0, collate_fn=self.collate_fn, drop_last=True
        )

    def val_dataloader(self):
        return DataLoader(
            self.val, batch_size=self.batch_size, num_workers=0, collate_fn=self.collate_fn, drop_last=True
        )
    ...
    def collate_fn(self, batch):
        ...
        # logger.info(f"audio_embed_len[0]: {audio_embed_lengths[0]}")
        # logger.info(f"exp_lengths[0]: {exp_lengths[0]}")
        # logger.info(f"pose_lengths[0]: {pose_lengths[0]}")
        # logger.info(f"shape_lengths[0]: {shape_lengths[0]}")
        # logger.info(f"landmarks3d_lengths[0]: {landmarks3d_lengths[0]}")
        ...
        packed_audio_embed = torch.nn.utils.rnn.pack_padded_sequence(
            padded_audio_embed, audio_embed_lengths, batch_first=True, enforce_sorted=False
        ).to(device)
        packed_exp = torch.nn.utils.rnn.pack_padded_sequence(
            padded_exp, exp_lengths, batch_first=True, enforce_sorted=False
        ).to(device)
        packed_pose = torch.nn.utils.rnn.pack_padded_sequence(
            padded_pose, pose_lengths, batch_first=True, enforce_sorted=False
        ).to(device)
        packed_shape = torch.nn.utils.rnn.pack_padded_sequence(
            padded_shape, shape_lengths, batch_first=True, enforce_sorted=False
        ).to(device)
        packed_landmarks3d = torch.nn.utils.rnn.pack_padded_sequence(
            padded_landmarks3d, landmarks3d_lengths, batch_first=True, enforce_sorted=False
        ).to(device)
...
if __name__ == '__main__':
    ...
    datamodule.setup()
    # device = 'cuda:0'
    logger.info(f'len: {len(datamodule.train)}')
    train_loader = datamodule.train_dataloader()
    #
    first_batch = next(iter(train_loader))
    packed_audio_embed, packed_exp, packed_pose, packed_shape, packed_landmarks3d = first_batch
    ...
    #
    # logger.info(f'sequence_lengths: {sequence_lengths}')
    #
    ...
    # i = 2
    # logger.info('-------- AFTER SLICING ----------')
    # logger.info(f"audio_embed shape: {unpacked_audio_embed[i].shape}")
    # logger.info(f"exp shape: {unpacked_exp[i].shape}")
    # logger.info(f"pose shape: {unpacked_pose[i].shape}")
    # logger.info(f"shape shape: {unpacked_shape[i].shape}")
    # logger.info(f"landmarks3d shape: {unpacked_landmarks3d[i].shape}")

    # dataset = NeuralRenderingDataset()
    # a = dataset[0]
    # logger.info(f'shape a: {a[0].shape}, {a[1].shape}, {a[2].shape}')
    ...
    # logger.info(f'masked_ref_images.shape: {masked_ref_images.shape}')
    # logger.info(f'orig_images.shape: {orig_images.shape}')
